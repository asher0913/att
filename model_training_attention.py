...
class MIA_train: # main class for every thing
    ...
    def __init__(self, arch, cutting_layer, batch_size, n_epochs,lambd=1, scheme="V2_epoch", num_client=1, dataset="cifar10",
                 ...
                 use_attention_classifier=False, num_slots=3, attention_heads=8, attention_dropout=0.1):
       ...
       # self.noniid_ratio = noniid
        ...
        if self.use_attention_classifier:
            ...
            # get_smashed_data_size() returns (batch_size, channels, height, width)
            # We only need the channels dimension for feature_dim
            feature_dim = self.model.get_smashed_data_size()[1]
    ...
    def attention_classify_features(self, features, labels):
        """
        Use attention classifier to classify features instead of GMM
        Args:
            features: (batch_size, channels, height, width) - features from the model
            labels: (batch_size,) - ground truth labels
        Returns:
            logits: (batch_size, num_classes) - classification logits
            enhanced_features: (batch_size, num_features, feature_dim) - enhanced features
            slot_representations: (batch_size, num_slots, feature_dim) - slot representations
        """
        if self.attention_classifier is None:
            raise ValueError("Attention classifier not initialized. Set use_attention_classifier=True")
        
        # Apply attention classification directly with 4D features
        # The attention_modules.py handles the reshaping internally
        logits, enhanced_features, slot_representations, attention_weights = self.attention_classifier(features)
    ...
    def train_target_step(self, x_private, label_private, adding_noise,random_ini_centers,centroids_list,client_id=0):
        ...
        # Use attention classifier if enabled, otherwise use GMM
        if self.use_attention_classifier:
        ...
        # Use attention classifier output if enabled, otherwise use original classifier
        if self.use_attention_classifier:
    ...
    def validate_target(self, client_id=0):
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            with torch.no_grad():
                # Use attention classifier if enabled, otherwise use original classifier
                if self.use_attention_classifier:
                    # Use attention classifier for feature classification
                    attention_logits, enhanced_features, slot_representations, attention_weights = self.attention_classify_features(output, target)
                    output = attention_logits
                else:
    ...
    def __call__(self, log_frequency=100, verbose=False, progress_bar=True):
        ...
        if not self.call_resume:
            ...
            if self.load_from_checkpoint:
                checkpoint_dir = "./pretrain_models/Vggs/C8_RES/"
                try:
    ...
    def pre_GAN_train(self, num_epochs, select_client_list=[0]):
        ...
        for client_id in select_client_list:
            ...
            self.attack(attack_num_epochs, decoder, optimizer,scheduler, trainloader, testloader, logger, path_dict,
                        attack_batchsize)
    ...
    def MIA_attack(self, num_epochs, attack_option="MIA", collude_client=1, target_client=0, noise_aware=False,
        ...
        if self.dataset == "cifar100":
            ...
            _, val_single_loader, _ = get_cifar10_trainloader(batch_size=1, num_workers=4, shuffle=True)
        elif self.dataset == "svhn":
            val_single_loader, _, _ = get_SVHN_testloader(batch_size=1, num_workers=4, shuffle=False)
        elif self.dataset == "mnist":
            _, val_single_loader = get_mnist_bothloader(batch_size=1, num_workers=4, shuffle=False)
        ...
        if ("MIA" in attack_option) and ("MIA_mf" not in attack_option):
            ...
            mse_score, ssim_score, psnr_score = self.test_attack(attack_num_epochs, decoder, sp_testloader, logger,
                                                                 path_dict, attack_batchsize,
                                                                 num_classes=self.num_class,sp_not=1)
            # Clear content of image_data_dir/tensor_data_dir
            if os.path.isdir(image_data_dir):
                rmtree(image_data_dir)
            if os.path.isdir(tensor_data_dir):
    ...
    def attack(self, num_epochs, decoder, optimizer,scheduler, trainloader, testloader, logger, path_dict, batch_size,
